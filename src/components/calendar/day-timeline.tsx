...
type DayTimelineProps = {
  ...
  options: FormOptions;
  /** Heading shown above the date. Defaults to "Meu Dia". */
  heading?: string;
  onDateChange: (date: Date) => void;
  onEventClick: (event: CalendarEventWithRelations) => void;
  /** Called after an event is created so the parent can refetch. */
  onChanged: () => void;
};
    ...
    <div className="space-y-6">
      ...
      <div className="overflow-x-auto rounded-xl border border-border bg-card">
        <div className="flex min-w-[280px]">
          <div className="w-11 shrink-0 border-r border-border sm:w-14">
            {hours.map((hour) => (
              <div
                key={hour}
                className="border-b border-border/50 pr-1.5 text-right text-[0.7rem] text-muted-foreground sm:pr-2 sm:text-xs"
